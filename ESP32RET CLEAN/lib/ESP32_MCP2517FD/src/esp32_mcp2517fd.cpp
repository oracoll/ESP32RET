...
#include "esp32_mcp2517fd_defines.h"
#include "esp32_mcp2517fd_regs.h"
...
uint16_t MCP2517FD::Read16(uint16_t address)
{
    ...
    digitalWrite(_CS,HIGH);
    SPI.endTransaction();
    uint16_t res = *((uint16_t *)(in_buff + 2));
    if (sdMutex != NULL) {
        xSemaphoreGiveRecursive(sdMutex);
    }
    return res;
...
uint32_t MCP2517FD::ReadFrameBuffer(uint16_t address, CAN_FRAME_FD &message) {
  ...
  */
    ...
    uint32_t res = (ptr_buff[1] >> 11) & 31; //return which filter produced this message
    if (sdMutex != NULL) {
        xSemaphoreGiveRecursive(sdMutex);
    }
    return res;
...
void MCP2517FD::intHandler(void) {
    ...
    extern SemaphoreHandle_t sdMutex;
    if (sdMutex != NULL) {
        if (xSemaphoreTakeRecursive(sdMutex, pdMS_TO_TICKS(1)) != pdTRUE) {
            return;
        }
    }
